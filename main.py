...
from prompts.entity_extraction import extract_entities
from prompts.spatial_planner import plan_spatial_queries
...
def main():
    ...
    with get_connection() as conn:
        ...
        for key, question in CHECKS.items():
            ...
            # 1) entity extraction ------------------------------------
            ent = extract_entities(question, all_objects, client)
            ...
            plan = plan_spatial_queries(
                ...
                targets=targets,
                queries={k: (v or "python_relation") for k, v in TEMPLATE_MAP.items()},
                client=client,
            )
            print("• Spatial‑planner calls:")
            print(json.dumps(plan.get("calls", []), indent=2, ensure_ascii=False))

            # 3) execute each call -----------------------------------
            executed_calls = []
            for idx, call in enumerate(plan.get("calls", []), start=1):
                call_result = run_spatial_call(
                    conn,
                    call,
                    template_paths={k: p for k, p in TEMPLATE_PATHS.items() if p},
                )
                executed_calls.append(call_result)

                status = call_result["status"]
                reason = call_result.get("reason")
                print(f"  └─ Call {idx}: {status}", end="")
                if reason:
                    print(f"  ({reason})")
                else:
                    # print first row (or count) for brevity
...
def main():
    ...
    print("KIAVE" + MY_OPENAI_KEY)
    client = openai

    # Load all objects and build lookup maps
    all_objects: List[Tuple[int, str, str]] = fetch_types_and_names()
    id_to_obj = {obj_id: (ifc_type, name) for obj_id, ifc_type, name in all_objects}
    all_ids = list(id_to_obj.keys())

    # Map IFC types → list of IDs
    ...
    with get_connection() as conn:
        for key, rule in CHECKS.items():
            ...
            print("Enriched Entities:", json.dumps(enriched, indent=2, ensure_ascii=False), "\n")

            # C) Plan
            plan = spatial_planner(enriched, TEMPLATE_CATALOGUE, client)
            print("Spatial plan:", json.dumps(plan, indent=2, ensure_ascii=False), "\n")

            # D) Execute relations in spatial plan and save only the positive ones
            positive_relations = []
            ...
            for entry in plan.get("plans", []):
                ...
                for tmpl in entry["templates"]:
                    ...
                    # Determine lists of IDs for 'a' and 'b'
                    ...
                    # Run and record only the 'held' cases
                    for a_id in a_ids:
                            ...
                            if rows:
                                ...
                                if tpl_name == "touches":
                                    ...
                                    relation_value = first[1]
                                        ...
                                        "front", "left", "right", "behind", "above", "below"
                                }:
                                    held = bool(first[3])
                                    relation_value = first[4] if held else None
                                elif tpl_name in {"near","far"}:
                                    # near_far.sql now returns [relation, distance, is_near, is_far]
            ...
            # Build essential summaries per reference entry
            summaries = []
            ...
            for entry in plan.get("plans", []):
                ...
                # describe what was checked against
                ...
                # pick reference items and labels
                ...
                for a_id, a_type, a_name, ref_label in ref_items:
                    # header: include against value too
                    ...
                    # now per‐relation detail lines
                    details = []
                    for tpl_name in templates_list:
                        # find positives for this ref & template
                        rels_tpl = []
                        for pr in positive_relations:
                            ...
                            if a_id is not None and pr["a_id"] != a_id:
                                continue
                            # category ref: all positive relations with this template
                            rels_tpl.append(pr)
            ...
            # E) Evaluate the rule compliance
            evaluation = evaluate_rule(rule, summaries, client)  # new call
    ...
    # Write all results including summaries
    with open("pipeline_output.json", "w", encoding="utf-8") as fp:
        json.dump(pipeline_results, fp, ensure_ascii=False, indent=2)

    print("\nDone! Summaries written to pipeline_output.json")
